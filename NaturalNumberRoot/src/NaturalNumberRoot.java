...
//      Get tooLow and tooHigh and maximize tooHigh
...
//      Incremented Low will be used for comparison because the number is always gonna be less then tooHigh so adding plus one will establish working comparison
        NaturalNumber incrementedLow = new NaturalNumber2(tooLow);
        incrementedLow.increment();

//      Loops through akgrathem till the smallest power is found
        while (incrementedLow.compareTo(tooHigh) < 0) {
...
//
            ...
            NaturalNumber midPower = new NaturalNumber2(mid);
            midPower.power(r);

//          does the comparison if midpower is smaller then n
            if (midPower.compareTo(n) > 0) {
                tooHigh.copyFrom(mid);
            } else {
                tooLow.copyFrom(mid);
                incrementedLow.copyFrom(tooLow);
                incrementedLow.increment();
            }
        }
