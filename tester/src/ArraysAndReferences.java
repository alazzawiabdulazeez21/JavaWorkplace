...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber productOfArrayElements(NaturalNumber[] nnArray) {
        ...
        NaturalNumber result = new NaturalNumber2(nnArray[nnArray.length - 1]);

        if (nnArray.length > 1) {
            result.multiply(productOfArrayElements(nnArray));
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < array.length; i++) {
            NaturalNumber temp = new NaturalNumber2(count);
            array[i] = temp;
            count.increment();
        }
        /*
