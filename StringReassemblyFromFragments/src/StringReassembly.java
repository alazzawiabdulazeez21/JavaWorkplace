...
//        Removed the last characters of the first string based on overall num
        String str1Removed = str1.substring(0, str1.length() - overlap);

//        Concatinates the removed str1 and str2
        String result = str1Removed + str2;
...
//        boolean to check if its a substring and a temp set
        boolean containStr = true;
        Set<String> temp = strSet.newInstance();

//        loops strSet until it becomes 0
        while (strSet.size() > 0) {
            /*
             * goes through the if statement if str is a substring of s then its
             * false then checks again if the if-else and if its still false
             * then adds it to temp
             */
        ...
        /*
         * transforms the string from temp back to strSet and add str if
         * containStr is still true
         */
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        /*
         * result will hold all the lines that arent substring of one another
         * with the help of method addToSetAvoidingSubstrings
         */
        Set<String> result = new Set1L<String>();
        while (!input.atEOS()) {
            String inputResult = input.nextLine();
            addToSetAvoidingSubstrings(result, inputResult);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        /*
         * using paramtive type character we can check if each character has '~'
         * and if it does it makes a line separator
         */
